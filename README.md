...
- Optional admin withdrawal approval can hold wallet-app withdrawals in
  `PENDING_REVIEW` before broadcast; wallet admin finance APIs expose summaries,
  deposit/withdrawal lists, and approve/reject actions.
...
| `GET /wallet/v1/admin/operations/overview` | Admin chain, RPC, scan height, hot-wallet, ledger and exception overview |
| `GET /wallet/v1/admin/operations/addresses` | Admin address ledger list filtered by chain, asset, user, role and address |
| `GET /wallet/v1/admin/operations/balances` | Admin non-zero ledger balances with address ownership metadata |
| `GET /wallet/v1/admin/operations/exceptions` | Admin deposit, withdrawal and collection exception queue |
| `GET /wallet/v1/admin/finance/summary` | Admin finance summary, asset totals, recent deposits and withdrawals |
| `GET /wallet/v1/admin/finance/deposits` | Admin deposit list filtered by chain, asset, status and credited flag |
| `GET /wallet/v1/admin/finance/withdrawals` | Admin withdrawal list filtered by chain, asset, status, user and order |
| `GET /wallet/v1/admin/finance/withdrawal-reviews` | Admin withdrawal review audit list filtered by chain, asset, decision, admin, user and order |
| `POST /wallet/v1/admin/finance/withdrawals/{chain}/{orderNo}/approve` | Approve a `PENDING_REVIEW` withdrawal and move it to `FROZEN` for signing |
| `POST /wallet/v1/admin/finance/withdrawals/{chain}/{orderNo}/reject` | Reject a `PENDING_REVIEW` withdrawal and release the frozen ledger amount |

Large admin lists now share the common cursor contract. `/wallet/v1/admin/operations/addresses`,
`/balances`, `/exceptions`, `/wallet/v1/admin/finance/deposits`, `/withdrawals`, and
`/withdrawal-reviews` accept `limit`, `cursor`, and `sort`. Operations and finance primary lists
support `updatedAt.desc` and `updatedAt.asc`; withdrawal review audit supports `createdAt.desc` and
`createdAt.asc`. The wrapped `data` payload returns `count`, the endpoint-specific list field,
`items`, `nextCursor`, `hasMore`, `sort`, and `limit`.

When admin approval is enabled, each approve/reject action writes `withdrawal_review_audit` with the reviewer identity, decision, previous and next status, reason, and released amount for rejected withdrawals.
