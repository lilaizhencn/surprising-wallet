...
| Conflux | 独立 L1，Core Space + eSpace | CFX | 原生币 | Conflux / EVM eSpace | [Docs](https://doc.confluxnetwork.org/) | 已接入；仅接入 eSpace（主网 `1030`、测试网 `71`），支持 Hardhat 与 EIP-7702；覆盖 CFX、USDT0、桥接 USDC，按约 `400` 个区块达到 PoS finality；Core Space 地址及交易模型不同，不纳入本次接入 |
