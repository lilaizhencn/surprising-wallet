...
> 三条扩展链均尚未完成有资金的公开测试网 E2E、独立审计和灰度，因此只能配置 `SHADOW`，主网必须保持禁用。实际操作分别以
...
> [Base 上线手册](./eip7702-base-production-runbook.md) 为准。
...
ETH/BNB/Polygon/Base 共用的第一阶段当前已经具备：
...
BNB、Polygon、Base 已分别完成 chainId 97、80002、84532 的本地真实 type-4 和生产路径测试；相应测试网至少一个 RPC
...
3. 广播前预占：L1 链为 `gasLimit * maxFeePerGas / 10^nativeDecimals`；OP Stack 链还必须在签名后调用 GasPriceOracle，
...
3. **项目层**：第 6 节的合约、Web3j 服务、账务和状态机完成并通过该网络端到端测试。ETH 已完成第一阶段；BNB、Polygon、
   Base 已分别完成 chainId 97、80002、84532 的本地真实 type-4 和两租户五地址生产路径。BNB 官方 RPC、Polygon 项目
