...
> 状态：目标设计，尚未实现。本文以仓库当前 `master` 代码为基线，给出可以按顺序实施的工程方案。
> 文中的 Solidity 是完整、可编译的参考实现，但涉及托管资金，完成独立审计、fork 测试和测试网灰度前不得用于生产。
...
- `EvmAccountTransactionService` 只构造 legacy ETH/ERC20 单笔交易；
- EVM 归集是一条 `collection_record` 对应一笔交易；
- 没有 delegate、BatchCollector、Relayer 和 Type-4 交易服务；
- Gas 预留按 21,000/65,000 固定估算，且 `custody_gas_usage` 强制绑定提现；
- 没有 Authority authorization nonce、delegate operation nonce 和 code hash 投影；
- 没有一笔 txHash 对应多个归集 item 的 Receipt 解析与对账模型。
...
3. 广播前预占：`reservedAmount = gasLimit * maxFeePerGas / 10^nativeDecimals`。
4. 最终确认后按回执的 `gasUsed * effectiveGasPrice` 结算，多占退回、少占补扣；余额不足进入 `OVERDUE`，但不能回滚
   已经发生的链上归集。
...
3. **项目层**：第 6 节的合约、Web3j 服务、账务和状态机完成并通过该网络端到端测试。目前这一层尚未实现，所以没有任何网络
   可以直接设为 `ACTIVE`。
...
| BNB | testnet | 97 | enabled | Pascal/BEP-441 已启用 | 接受 type-4，进入 nonce 校验 | 可做第一优先级 SHADOW/E2E |
| BNB | mainnet | 56 | disabled | Pascal/BEP-441 已启用 | 接受 type-4，进入 Gas 价格校验 | 测试网通过后再开 SHADOW |
| POLYGON | amoy | 80002 | enabled | Bhilai/PIP-51 体系 | 接受 type-4，进入最低 tip 校验 | 可做 SHADOW/E2E |
| POLYGON | mainnet | 137 | disabled | Bhilai 于 2025-07-01 上线 | 接受 type-4，进入最低 tip 校验 | Amoy 通过后再开 SHADOW |
| ARBITRUM | sepolia | 421614 | enabled | ArbOS 40 Callisto | 接受 type-4，进入 nonce 校验 | 可做 SHADOW/E2E |
...
| BASE | sepolia | 84532 | enabled | Isthmus 于 2025-04-17 激活 | 接受 type-4，进入 nonce 校验 | 可做第一优先级 SHADOW/E2E |
...
- Arbitrum：[ArbOS 40 Callisto](https://blog.arbitrum.foundation/the-smartest-wallet-you-already-own-is-on-arbitrum/)，明确说明
  Arbitrum One 已于 2025-06-18 获得 EIP-7702 支持。
