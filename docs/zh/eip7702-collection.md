...
> 状态：ETH 第一阶段已实现并通过 Hardhat Prague + PostgreSQL 生产路径集成测试；尚未完成独立合约审计和 Sepolia 灰度，
> 因此主网必须保持禁用。本文保留协议设计和逐链能力矩阵；实际部署、启用、监控、暂停和回滚以
> [ETH EIP-7702 生产上线手册](./eip7702-eth-production-runbook.md) 为准。
...
尚未完成的生产门禁是第三方合约审计、Sepolia 灰度和主网变更审批；Console 批次详情与运维 API 属于后续可视化工作，
不影响当前后台归集闭环。
