...
租户请求一条链的新充值地址时，只提交链代码 `chainId`。服务从该链当前唯一启用的
...
`chainId` 是链代码，例如 `ETH`、`BTC`、`SOLANA`，不是 EVM 数值 chain ID。每个新的
`Idempotency-Key` 分配一个新地址；同一个键的安全重试返回同一结果。租户必须保存返回的地址 ID
和地址，并在收到充值 Webhook 时按地址完成自己的客户映射。
...
`tenantNamespace`。每个新地址从全局 `custody_derivation_subject_seq` 获得唯一的
`derivationSubject`。secp256k1/Bitcoin-like 地址使用以下非 hardened、自定义 BIP44 形态路径：
...
递增。公开 API 每次新分配都会获得新的 `derivationSubject`，所以正常的新地址的
`childIndex` 为 0；该层仍保留用于同一派生 subject 下的后续地址。
