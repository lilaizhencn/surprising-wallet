...
| `GET /wallet/v1/admin/operations/overview` | 后台链、RPC、扫描高度、热钱包、账本和异常聚合总览 |
| `GET /wallet/v1/admin/operations/addresses` | 后台地址账本列表，支持链、资产、用户、角色和地址过滤 |
| `GET /wallet/v1/admin/operations/balances` | 后台非零余额账户列表，附带地址归属信息 |
| `GET /wallet/v1/admin/operations/exceptions` | 后台充值、提现、归集异常队列 |
| `GET /wallet/v1/admin/finance/summary` | 后台财务汇总、资产维度统计、最近充值和提现 |
| `GET /wallet/v1/admin/finance/deposits` | 后台充值列表，支持链、资产、状态和是否入账过滤 |
| `GET /wallet/v1/admin/finance/withdrawals` | 后台提现列表，支持链、资产、状态、用户和订单过滤 |
| `GET /wallet/v1/admin/finance/withdrawal-reviews` | 后台提现审核审计列表，支持链、资产、审核结果、管理员、用户和订单过滤 |
| `POST /wallet/v1/admin/finance/withdrawals/{chain}/{orderNo}/approve` | 审核通过 `PENDING_REVIEW` 提现，转为 `FROZEN` 后进入签名出款流程 |
| `POST /wallet/v1/admin/finance/withdrawals/{chain}/{orderNo}/reject` | 拒绝 `PENDING_REVIEW` 提现，释放已冻结账本金额并标记 `REJECTED` |
...
后台大列表已经接入统一游标分页：`/wallet/v1/admin/operations/addresses`、`/balances`、`/exceptions`、`/wallet/v1/admin/finance/deposits`、`/withdrawals` 和 `/withdrawal-reviews` 支持 `limit`、`cursor`、`sort`。operations/finance 主列表支持 `updatedAt.desc`、`updatedAt.asc`，提现审核审计支持 `createdAt.desc`、`createdAt.asc`；响应数据在 `data` 中返回 `count`、对应列表字段、`items`、`nextCursor`、`hasMore`、`sort`、`limit`。

后台配置账号密码来自 `SW_WALLET_ADMIN_USERNAME`、`SW_WALLET_ADMIN_PASSWORD`，生产环境必须覆盖默认值。`wallet_system_config` 中的 `withdrawal.admin.approval.required` 设为 `true` 后，用户提现会先冻结账本金额并停在 `PENDING_REVIEW`，只有后台审核通过后才会进入链上签名广播流程。每次通过/拒绝都会写入 `withdrawal_review_audit`，保留审核人、审核结果、前后状态、原因和拒绝释放金额。
