...
- ETH Relayer 地址及其在 `chain_address` 中可由 wallet-api 根密钥派生的记录；
...
第一阶段允许复用已经由 wallet-api 验证过派生路径的 ETH 平台地址作为链级 Relayer。先只读检查，结果必须恰好一行：
...
4. 回滚 wallet-api 版本，但保留新表和历史数据只读，不能删除审计证据。
