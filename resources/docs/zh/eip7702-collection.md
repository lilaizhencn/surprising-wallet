...
> Optimism 阶段已通过 chainId 11155420 的同组门禁，并复用经 Base/OP Sepolia 分别验证的 OP Stack 完整费用模型。
> 五条扩展链均尚未完成有资金的公开测试网 E2E、独立审计和灰度，因此只能配置 `SHADOW`，主网必须保持禁用。实际操作分别以
...
> [Base 上线手册](eip7702-base-production-runbook.md)、[Arbitrum 上线手册](eip7702-arbitrum-production-runbook.md)、
> [Optimism 上线手册](eip7702-optimism-production-runbook.md) 为准。
...
ETH/BNB/Polygon/Base/Arbitrum/Optimism 共用的第一阶段当前已经具备：

- `../../../pom.xml` 使用 Web3j `6.0.0`，依赖中已有 `Transaction7702` 和 `AuthorizationTuple`；
- `resources/infra/evm-fork` 使用 Hardhat `2.26.3`，包含 Prague/EIP-7702 交易字段；
...
BNB、Polygon、Base、Arbitrum、Optimism 已分别完成 chainId 97、80002、84532、421614、11155420 的本地真实 type-4 和
生产路径测试；相应测试网至少一个 RPC
...
修改 `..b/surprising-wallet-init-pgsql.sql`，增加 `evm_7702_config`。目标 DDL：
...
1. 创建 `resources/infra/resources/infra/evm-fork/contracts/Eip7702Collection.sol`，内容使用第 5 节完整代码。
2. 修改 `resources/infra/evm-fork/hardhat.config.js`：
...
创建 `resources/infra/evm-fork/scripts/deploy-eip7702-collection.js`：
...
- admin 使用平台多签，不使用 wallet-api 在线密钥；
...
实际开发时直接修改 `..b/surprising-wallet-init-pgsql.sql` 的建表语句，并重建开发库；上面的 SQL 只用于说明目标变化，
...
1. `wallet-api/.../jobs/custody/CustodyRepository.java`：所有查询必须同时带 `tenant_id + operation_type + operation_id`，
   `custody_ledger_entry.reference_type` 对归集写 `COLLECTION_BATCH`，`reference_id` 写批次号。
2. `wallet-api/.../jobs/custody/CustodyGasService.java`：保留提现估算入口，新增批次估算入口；不要继续用当前 ERC-20
...
在 `wallet-api/src/main/java/com/surprising/wallet/jobs/account/` 增加
...
在 `resources/infra/evm-fork/test/Eip7702Collection.test.js` 至少覆盖：
...
3. **项目层**：第 6 节的合约、Web3j 服务、账务和状态机完成并通过该网络端到端测试。ETH 已完成第一阶段；BNB、Polygon、
   Base、Arbitrum、Optimism 已分别完成 chainId 97、80002、84532、421614、11155420 的本地真实 type-4 和两租户五地址生产路径。
   BNB 官方 RPC、Polygon 项目 PublicNode，以及 Base、Arbitrum、Optimism 的官方/项目 RPC 均通过严格 estimate 门禁；Base 与
   Optimism 完成 OP Stack 完整费用入账，Arbitrum 完成 Nitro 父链费用拆分。五条链都还没有有资金的公开测试网 E2E。其他链仍须
   逐链集成。没有任何网络可以因此直接设为生产 `ACTIVE`。
...
| OPTIMISM | sepolia | 11155420 | enabled | Isthmus；当前 Jovian | 官方/项目 RPC 严格 estimate、只读 L1/GasPriceOracle、本地真实 type-4/生产路径通过 | 已集成；仅 SHADOW，需双生产 RPC 和有资金 E2E |
