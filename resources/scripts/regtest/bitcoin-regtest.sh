...
IMAGE=${BTC_REGTEST_IMAGE:-surprising-wallet/bitcoin-core:29.0}
CONTAINER=${BTC_REGTEST_CONTAINER:-surprising-wallet-btc-regtest}
VOLUME=${BTC_REGTEST_VOLUME:-surprising-wallet-btc-regtest-data}
RPC_USER=${BTC_REGTEST_RPC_USER:-wallet}
RPC_PASSWORD=${BTC_REGTEST_RPC_PASSWORD:-wallet123}
RPC_PORT=${BTC_REGTEST_RPC_PORT:-18444}
WALLET=${BTC_REGTEST_WALLET:-regtest-funder}
...
start() {
  ...
  docker build --platform linux/amd64 \
    -t "${IMAGE}" \
    "${ROOT_DIR}/infra/regtest/bitcoin"
  if docker container inspect "${CONTAINER}" >/dev/null 2>&1; then
    ...
    docker run -d \
      ...
      -p "127.0.0.1:${RPC_PORT}:18446" \
      -e BTC_REGTEST_RPC_USER="${RPC_USER}" \
      -e BTC_REGTEST_RPC_PASSWORD="${RPC_PASSWORD}" \
      -v "${VOLUME}:/var/lib/bitcoin" \
      "${IMAGE}" >/dev/null
  fi
  wait_ready
