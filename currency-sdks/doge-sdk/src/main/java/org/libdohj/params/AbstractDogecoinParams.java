...
public abstract class AbstractDogecoinParams extends NetworkParameters implements AuxPoWNetworkParameters {
    ...
    public AbstractDogecoinParams(final int setDiffChangeTarget) {
        ...
        this.newInterval = DOGE_INTERVAL_NEW;
        this.targetTimespan = DOGE_TARGET_TIMESPAN;
        this.newTargetTimespan = DOGE_TARGET_TIMESPAN_NEW;
        this.maxTarget = Utils.decodeCompactBits(0x1e0fffffL);
        this.diffChangeTarget = setDiffChangeTarget;

        this.packetMagic = 0xc0c0c0c0;
//        this.bip32HeaderpPub = 0x0488C42E; //The 4 byte header that serializes in base58 to "xpub". (?)
//        this.bip32HeaderPriv = 0x0488E1F4; //The 4 byte header that serializes in base58 to "xprv" (?)
        this.bip32HeaderP2PKHpub =0x0488C42E;
        this.bip32HeaderP2PKHpriv = 0x0488E1F4;
    }

    private static AltcoinBlock createGenesis(final NetworkParameters params) {
        final AltcoinBlock genesisBlock = new AltcoinBlock(params, Block.BLOCK_VERSION_GENESIS);
        final Transaction t = new Transaction(params);
        try {
            final byte[] bytes = Utils.HEX.decode
    ...
    public Coin getBlockSubsidy(final int height) {
        if (height < DIGISHIELD_BLOCK_HEIGHT) {
            ...
            return BASE_SUBSIDY.shiftRight(height / this.getSubsidyDecreaseBlockCount()).multiply(2);
    ...
    public void checkDifficultyTransitions(final StoredBlock storedPrev, final Block nextBlock, final BlockStore blockStore)
        ...
        try {
            final long newTargetCompact = this.calculateNewDifficultyTarget(storedPrev, nextBlock, blockStore);
        ...
        } catch (final CheckpointEncounteredException ex) {
     ...
     *                                        be provided.
     */
    public long calculateNewDifficultyTarget(final StoredBlock storedPrev, final Block nextBlock, final BlockStore blockStore)
        ...
        if ((storedPrev.getHeight() + 1) % retargetInterval != 0) {
            if (this.allowMinDifficultyBlocks()) {
                ...
                // then allow mining of a min-difficulty block.
                if (nextBlock.getTimeSeconds() > prev.getTimeSeconds() + this.getTargetSpacing(previousHeight + 1) * 2) {
                    ...
                    while (cursor.getHeight() % retargetInterval != 0
                        ...
                        final StoredBlock prevCursor = cursor.getPrev(blockStore);
                        if (prevCursor == null) {
                            break;
                        }
                        cursor = prevCursor;
        ...
        final Block blockIntervalAgo = cursor.getHeader();
     ...
     */
    protected long calculateNewDifficultyTargetInner(final int previousHeight, final Block prev,
     ...
     * @return New difficulty target as compact bytes.
     */
    protected long calculateNewDifficultyTargetInner(final int previousHeight, final long previousBlockTime,
        ...
        if (newTarget.compareTo(this.getMaxTarget()) > 0) {
            this.log.info("Difficulty hit proof of work limit: {}", newTarget.toString(16));
            newTarget = this.getMaxTarget();
        }

        final int accuracyBytes = (int) (nextDifficultyTarget >>> 24) - 3;
     ...
     */
    @Override
    public Sha256Hash getBlockDifficultyHash(final Block block) {
        return ((AltcoinBlock) block).getScryptHash();
    }

    @Override
    public AltcoinSerializer getSerializer(final boolean parseRetain) {
     ...
     */
    protected int getTargetSpacing(final int height) {
        final boolean digishieldAlgorithm = height >= this.getDigishieldBlockHeight();
        final int retargetInterval = digishieldAlgorithm
                ? this.getNewInterval()
                : this.getInterval();
