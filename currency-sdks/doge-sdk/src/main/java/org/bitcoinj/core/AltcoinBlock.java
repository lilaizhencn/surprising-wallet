...
public class AltcoinBlock extends Block {
    ...
    public AltcoinBlock(final NetworkParameters params, final byte[] payloadBytes, final int offset,
                        final Message parent, final MessageSerializer serializer, final int length)
            throws ProtocolException {
     ...
     */
    public AltcoinBlock(final NetworkParameters params, final long version, final Sha256Hash prevBlockHash, final Sha256Hash merkleRoot, final long time,
                        final long difficultyTarget, final long nonce, final List<Transaction> transactions) {
    ...
    private Sha256Hash calculateScryptHash() {
        try {
            ...
            return Sha256Hash.of(reverseBytes(scryptDigest(bos.toByteArray())));
    ...
    public void setAuxPoW(final AuxPoW auxpow) {
        this.auxpow = auxpow;
    }
    ...
    @Override
    public Coin getBlockInflation(final int height) {
    ...
    protected void parseAuxPoW() throws ProtocolException {
        ...
        if (this.auxpowChain) {
            ...
            if (auxpowParams.isAuxPoWBlockVersion(this.getRawVersion())
                    && this.payload.length >= 160) { // We have at least 2 headers in an Aux block. Workaround for StoredBlocks
    ...
    @Override
    void writeHeader(final OutputStream stream) throws IOException {
    ...
    public Block cloneAsHeader() {
        final AltcoinBlock block = new AltcoinBlock(this.params, this.getRawVersion());
     ...
     */
    @Override
    protected boolean checkProofOfWork(final boolean throwException) throws VerificationException {
        if (this.params instanceof AltcoinNetworkParameters) {
            final BigInteger target = this.getDifficultyTargetAsInteger();
            ...
            if (this.params instanceof AuxPoWNetworkParameters) {
                ...
                if (auxParams.isAuxPoWBlockVersion(this.getRawVersion()) && null != this.auxpow) {
                    return this.auxpow.checkProofOfWork(this.getHash(), target, throwException);
                }
            }

            final AltcoinNetworkParameters altParams = (AltcoinNetworkParameters) this.params;
            final BigInteger h = altParams.getBlockDifficultyHash(this).toBigInteger();
            if (h.compareTo(target) > 0) {
                // Proof of work check failed!
                if (throwException) {
                    throw new VerificationException("Hash is higher than target: " + this.getHashAsString() + " vs "
                            + target.toString(16));
                } else {
                    return false;
                }
