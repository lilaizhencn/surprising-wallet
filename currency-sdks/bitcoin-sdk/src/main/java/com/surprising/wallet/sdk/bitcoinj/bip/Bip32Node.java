...
public class Bip32Node {
    ...
    public static Bip32Node getMasterKey(byte[] seed) {
        ...
        if (bigInt.compareTo(CURVE.getN()) >= 0) {
    ...
    public static Bip32Node getChildNode(Bip32Node node, int sequence) {
        ...
        if (nodeKey.hasPrivKey()) {
            ...
            ECPoint point = CURVE.getG().multiply(bigInt)
                    .add(CURVE.getCurve().decodePoint(pubKey));
            if (point.isInfinity()) {
                throw new RuntimeException("生成了一个不应该出现的数值！");
            }
            //新版本与旧版本的传参不同，X = point.normalize().getXCoord() Y = point.normalize().getYCoord()
            pubKey = new ECPoint.Fp(ECKey.CURVE.getCurve(), point.normalize().getXCoord(), point.normalize().getYCoord(), true)
                    .getEncoded(true);
    ...
    private String serialize(int coinType, boolean isMainNet, boolean isPrivate) {
        ...
        System.arraycopy(chainCode, 0, result, 13, 32);
        pos += 32;
