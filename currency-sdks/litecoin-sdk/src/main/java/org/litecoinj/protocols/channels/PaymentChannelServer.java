...
public class PaymentChannelServer {
    ...
    private void settlePayment(final CloseReason clientRequestedClose) throws InsufficientMoneyException {
        ...
        if (keyFuture != null) {
            //TODO 此处修改可能有很大问题
            result = Futures.transform(conn.getUserKey(), new Function<KeyParameter, Transaction>() {
                @Nullable
                @Override
                public Transaction apply(@Nullable KeyParameter userKey) {
                    try {
                        return state.close(userKey).get();
                    }catch (InterruptedException | InsufficientMoneyException | ExecutionException e) {
                        e.printStackTrace();
                    }
                    return null;
                }
