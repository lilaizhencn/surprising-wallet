...
import org.tron.wallet.crypto.jce.*;
import org.tron.wallet.util.BIUtil;
import org.tron.wallet.util.ByteUtil;
...
public class ECKey implements Serializable {
     ...
     * @return -
     */
    public static ECKey fromPrivateAndPrecalculatedPublic(final BigInteger priv,
                                                          final ECPoint pub) {
     ...
     * @return -
     */
    public static ECKey fromPrivateAndPrecalculatedPublic(final byte[] priv, final byte[]
            pub) {
     ...
     * @return -
     */
    public static byte[] publicKeyFromPrivate(final BigInteger privKey, final boolean
            compressed) {
     ...
     */
    public static byte[] computeAddress(final byte[] pubBytes) {
     ...
     * @return 21-byte address
     */
    public static byte[] computeAddress(final ECPoint pubPoint) {
     ...
     */
    public static byte[] pubBytesWithoutFormat(final ECPoint pubPoint) {
    ...
    public static byte[] signatureToKeyBytes(final byte[] messageHash, final String
    ...
    public static byte[] signatureToKeyBytes(final byte[] messageHash,
                                             final ECDSASignature sig) throws
            SignatureException {
        ...
        final byte[] key = ECKey.recoverPubBytesFromSignature(recId, sig,
                messageHash);
        if (key == null) {
     ...
     */
    public static byte[] signatureToAddress(final byte[] messageHash, final String
            signatureBase64) throws SignatureException {
     ...
     * @return 20-byte address
     */
    public static byte[] signatureToAddress(final byte[] messageHash,
                                            final ECDSASignature sig) throws
            SignatureException {
     ...
     * @return ECKey
     */
    public static ECKey signatureToKey(final byte[] messageHash, final String
            signatureBase64) throws SignatureException {
     ...
     * @return ECKey
     */
    public static ECKey signatureToKey(final byte[] messageHash, final ECDSASignature
     ...
     * @return -
     */
    public static boolean verify(final byte[] data, final ECDSASignature signature,
                                 final byte[] pub) {
        final ECDSASigner signer = new ECDSASigner();
        final ECPublicKeyParameters params = new ECPublicKeyParameters(CURVE
     ...
     * @return -
     */
    public static boolean verify(final byte[] data, final byte[] signature, final byte[] pub) {
     ...
     */
    @Nullable
    public static byte[] recoverPubBytesFromSignature(final int recId,
        ...
        final BigInteger prime = curve.getQ();  // Bouncy Castle is not consistent
        ...
        final ECPoint.Fp q = (ECPoint.Fp) ECAlgorithms.sumOfTwoMultiplies(CURVE
     ...
     */
    @Nullable
    public static byte[] recoverAddressFromSignature(final int recId,
                                                     final ECDSASignature sig,
                                                     final byte[] messageHash) {
     ...
     */
    @Nullable
    public static ECKey recoverFromSignature(final int recId, final ECDSASignature sig,
                                             final byte[] messageHash) {
        final byte[] pubBytes = recoverPubBytesFromSignature(recId, sig,
                messageHash);
        if (pubBytes == null) {
    ...
    private static ECPoint decompressKey(final BigInteger xBN, final boolean yBit) {
        ...
        final byte[] compEnc = x9.integerToBytes(xBN, 1 + x9.getByteLength(CURVE
                .getCurve()));
    ...
    public ECKey decompress() {
        if (!this.pub.isCompressed()) {
            ...
            return new ECKey(this.provider, this.privKey, decompressPoint(this.pub));
        }
    }
    ...
    public ECKey compress() {
        if (this.pub.isCompressed()) {
            ...
            return new ECKey(this.provider, this.privKey, compressPoint(this.pub));
        }
    }
    ...
    public byte[] getPubKey() {
        return this.pub.getEncoded(/* compressed */ false);
    }
    ...
    public ECDSASignature doSign(final byte[] input) {
        ...
        if (this.privKey instanceof BCECPrivateKey) {
            final ECDSASigner signer = new ECDSASigner(new HMacDSAKCalculator(new
                    SHA256Digest()));
            final ECPrivateKeyParameters privKeyParams = new ECPrivateKeyParameters
                    (((BCECPrivateKey) this.privKey).getD(), CURVE);
    ...
    public ECDSASignature sign(final byte[] messageHash) {
        final ECDSASignature sig = this.doSign(messageHash);
    ...
    public BigInteger keyAgreement(final ECPoint otherParty) {
        if (this.privKey == null) {
            ...
            } catch (final IllegalStateException | InvalidKeyException |
    ...
    public byte[] decryptAES(final byte[] cipher) {
        ...
        final KeyParameter key = new KeyParameter(BigIntegers.asUnsignedByteArray((
                (BCECPrivateKey) this.privKey).getD()));
    ...
    public boolean isPubKeyCanonical() {
        return isPubKeyCanonical(this.pub.getEncoded(/* uncompressed */ false));
    }
    ...
    public static class ECDSASignature {
         ...
         * @return -
         */
        public static ECDSASignature fromComponents(final byte[] r, final byte[] s, final byte
                v) {
        ...
        public static boolean validateComponents(final BigInteger r, final BigInteger s,
                                                 final byte v) {
        ...
        public String toBase64() {
            final byte[] sigData = new byte[65];  // 1 header + 32 bytes for R + 32
            // bytes for S
            sigData[0] = this.v;
