...
public class BchSecondSignService implements ISignService {
    ...
    public String signTransaction(WithdrawTransaction transaction) {
        ...
        try {
            if (!"bch-p2sh".equals(JacksonJson.text(signature, "scriptType"))) {
                throw new IllegalArgumentException("not BCH P2SH");
            }
            List<Address> addresses =
                    JacksonJson.toList(objectMapper, signature.get("addresses"), Address.class);
            List<UtxoTransaction> utxos =
                    JacksonJson.toList(objectMapper, signature.get("utxos"), UtxoTransaction.class);
            List<String> redeemScripts =
                    JacksonJson.toList(objectMapper, signature.get("redeemScripts"), String.class);
            ...
            return builder.buildSecondSign(
                    JacksonJson.text(signature, "firstSignTx"), keys, redeemScripts);
