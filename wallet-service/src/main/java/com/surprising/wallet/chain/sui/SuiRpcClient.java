...
/**
 * Sui 链 gRPC 客户端，封装对 Sui FullNode gRPC 接口的调用。
 *
 * <p>通过 Sui 的 LedgerService、StateService、TransactionExecutionService 三个 gRPC 服务
 * 提供区块查询、余额查询、交易执行等功能。与 Aptos REST 接口不同，Sui 使用 gRPC 协议。</p>
 *
 * <p>支持多节点故障转移，并自动将 gRPC 响应转换为 JSON 格式以兼容下游处理。</p>
 *
 * @see SuiDepositScanner
 * @see SuiTransactionService
 */
@Component
public
class SuiRpcClient {
