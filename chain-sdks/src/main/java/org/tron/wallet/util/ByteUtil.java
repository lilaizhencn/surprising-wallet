...
public class ByteUtil {
     ...
     */
    public static int numberOfLeadingZeros(final byte[] bytes) {

        final int i = firstNonZeroByte(bytes);

        if (i == -1) {
            return bytes.length * 8;
        } else {
            final int byteLeadingZeros = Integer.numberOfLeadingZeros((int) bytes[i] & 0xff) - 24;
     ...
     * returned
     */
    public static byte[] parseBytes(final byte[] input, final int offset, final int len) {
     ...
     */
    public static byte[] parseWord(final byte[] input, final int idx) {
     ...
     */
    public static byte[] parseWord(final byte[] input, final int offset, final int idx) {
        return parseBytes(input, offset + 32 * idx, 32);
    }

    public static boolean greater(final byte[] bytes1, final byte[] bytes2) {
        return compare(bytes1, bytes2) > 0;
    }

    public static boolean greaterOrEquals(final byte[] bytes1, final byte[] bytes2) {
        return compare(bytes1, bytes2) >= 0;
    }

    public static boolean less(final byte[] bytes1, final byte[] bytes2) {
        return compare(bytes1, bytes2) < 0;
    }

    public static boolean lessOrEquals(final byte[] bytes1, final byte[] bytes2) {
        return compare(bytes1, bytes2) <= 0;
    }

    public static boolean equals(final byte[] bytes1, final byte[] bytes2) {
        return compare(bytes1, bytes2) == 0;
    }

    // lexicographical order
    private static int compare(final byte[] bytes1, final byte[] bytes2) {
        ...
        for (int i = 0; i < length; ++i) {
            final int ret = UnsignedBytes.compare(bytes1[i], bytes2[i]);
            if (ret != 0) {
                return ret;
            }
        }
