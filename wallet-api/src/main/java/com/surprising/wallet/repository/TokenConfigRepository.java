...
public class TokenConfigRepository {
    ...
    public long insert(String chain, String network, String symbol, String standard,
                       ...
                       BigDecimal minDeposit, BigDecimal minWithdraw, boolean collectEnabled,
        ...
        Long id = jdbc.queryForObject("""
                insert into token_config(
                    ...
                    collect_enabled, collect_threshold, gas_strategy, confirmation_required,
                    updated_at)
                values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, now()) returning id
                """, Long.class, chain, network, symbol, standard, standard, contractAddress,
                contractAddressBase58, contractAddressHex, decimals, enabled, minDeposit,
                minWithdraw, minDeposit, minWithdraw, collectEnabled, collectThreshold,
                gasStrategy, confirmationRequired);
        return id;
    }
    ...
    public int update(long id, String chain, String network, String symbol, String standard,
        ...
        return jdbc.update("""
                update token_config
                 ...
                 where id = ?
                """, chain, network, symbol, standard, standard, contractAddress,
                contractAddressBase58, contractAddressHex, decimals, enabled, minDeposit,
                minWithdraw, minDeposit, minWithdraw, collectEnabled, collectThreshold,
                gasStrategy, confirmationRequired, id);
    }

    /** 修改代币启用状态。 */
    public int updateEnabled(long id, boolean enabled, boolean collectEnabled) {
        return jdbc.update("""
                update token_config set enabled = ?, collect_enabled = ?, updated_at = now()
                 where id = ?
    ...
    public long countByChain(String chain) {
        Long count = jdbc.queryForObject(
                "select count(*) from token_config where upper(chain) = upper(?)",
                Long.class, chain);
        return count == null ? 0 : count;
    }

    /** 判断指定链和资产是否存在运行数据。 */
    public boolean existsByChainAndSymbol(String chain, String symbol) {
        return !jdbc.queryForList("""
                select id from token_config
                 where upper(chain) = upper(?) and upper(symbol) = upper(?) limit 1
